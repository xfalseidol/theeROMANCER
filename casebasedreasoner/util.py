...
def make_graphviz_graph(cbrinst, include_inheritance_edges=True, include_slot_edges=True):
    ...
    g = ["digraph G {", "rankdir=LR", "splines=true", "overlap=false", "layout=dot"]
    mopname_to_nodename = {}
    curr_nodeid = 0
    ...
    for mopname in cbrinst.mops:
        ...
        thislabel = [
            f"<table cellspacing=\"0\" bgcolor=\"{moptype_to_color[this_mop.mop_type]}\"><tr><td colspan=\"2\" id=\"n\">{str(mopname)}</td></tr>"]
        slotnum = 0
        ...
        l = "".join(thislabel)
        g.append(f"{curr_nodename} [shape=none label=<{l}>];")
